...
pub mod step_staking {
    ...
    // DEVIS-EDITED
    pub fn stake(ctx: Context<Stake>, nonce: u8, amount: u64, end_time: i64, fee: u64) -> ProgramResult {
    ...
    pub fn unstake(ctx: Context<Unstake>, nonce: u8, amount: u64) -> ProgramResult {
        ...
        let stake_data = &mut ctx.accounts.stake_data;
        let amount_leave_valut: u64;
        if now_time < stake_data.end_time { // Unstake before end time
            amount_leave_valut = (what as u128)
                ...
                .try_into()
                .unwrap();
        } else {                            // Unstake after end time
            amount_leave_valut = 0;
        }
        let amount_to_staker: u64 = what - amount_leave_valut;
        ...
        // DEVIS-EDITED
        token::transfer(cpi_ctx, amount_to_staker)?;
...
pub struct Stake<'info> {
    ...
    // DEVIS-EDITED
    #[account(init, payer=token_from_authority, space= 8 + STAKE_DATA_SIZE)]
...
// DEVIS-EDITED
pub const STAKE_DATA_SIZE : usize = 32 + 8;
